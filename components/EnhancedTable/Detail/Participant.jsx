...
export default function Participant({ detail, onClose, fetchRows }) {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const res = await axios.post("/api/common/participant/read?e=admin", {
          idString: detail,
        });
        if (res?.data) {
  ...
  const handleUpdate = async () => {
    ...
    setIsLoading(true);
    try {
      await axios.post("/api/common/participant/update?e=admin", {
        idString: detail,
        participant: data,
      });
  ...
  const handleArchive = async () => {
    setIsLoading(true);
    try {
      await axios.post("/api/common/participant/archive?e=admin", {
        idString: detail,
      });
    } catch (err) {
      console.error(err);
